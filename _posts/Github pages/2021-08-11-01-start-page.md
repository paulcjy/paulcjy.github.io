...
Github 블로그는 repository를 이용해서 관리한다. 주의할 점은, repository 이름이 *username*.github.io일 때에만 블로그로 사용이 가능하다. *username*에는 본인의 계정 이름을 넣으면 된다. 그러고 여기에 내용물이 되는 파일들을 업로드한다.
